...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        DB.autor = AutorFirestore()
        DB.libro = LibroFirestore()
...
//        loadAutores()
    ...
    private fun createAutorFromDocument(document: QueryDocumentSnapshot): Autor {
        val id = document.id
    ...
    private fun loadAutores() {
        ...
        autors = arrayListOf<Autor>()
        DB.autor!!.getAll()
            .addOnSuccessListener {result ->
                for (document in result) {
                    val autor = createAutorFromDocument(document)
                    autors!!.add(autor)
                }
                if (autors != null) {
    ...
    override fun onContextItemSelected(item: MenuItem): Boolean {
        ...
        return when(item.itemId) {
            R.id.mi_show_series -> {
                ...
                params.putLong("anioNacimiento", autor.anioNacimiento)
